...
} from "../../src/commands/export-schema";
...
describe("export-schema command", () => {
  ...
  beforeEach(async () => {
    ...
    // Setup SDK mocks
    const sdkModule = await import("@mimsy-cms/sdk");
  ...
  describe("command registration", () => {
    it("should register the export-schema command with options", () => {
      ...
      const exportCmd = program.commands.find(
        (cmd) => cmd.name() === "export-schema",
      );
      expect(exportCmd).toBeDefined();
      expect(exportCmd?.description()).toBe(
        "Export collection schemas to a JSON file",
      );
  ...
  describe("exportSchemaAction", () => {
    it("should export schema successfully with default options", async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining("Schema exported successfully"),
      );
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining("Collections exported: 2"),
      );
    });
    ...
    it("should handle export errors gracefully", async () => {
      ...
      await expect(exportSchemaAction(options)).rejects.toThrow(
        "process.exit called",
      );
    ...
    it("should parse command line arguments correctly", async () => {
      ...
      try {
        await program.parseAsync(
          ["export-schema", "--output", "custom.json", "--pretty", "--clear"],
          { from: "user" },
        );
      } catch (e) {
