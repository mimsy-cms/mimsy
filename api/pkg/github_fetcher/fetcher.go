...
type GithubFetcher interface {
	...
	GetRepositoryContents(ctx context.Context, repository string) (*zip.Reader, error)
}
...
func (f *githubFetcher) GetRepositoryContents(ctx context.Context, repository string) (*zip.Reader, error) {
	owner, repo, err := parseRepository(repository)
	if err != nil {
		return nil, err
	}
	...
	zipReader, err := downloadZipArchive(ctx, client, owner, repo, "")
	if err != nil {
