...
test("collections with builtins", () => {
  ...
  expectTypeOf(test).toEqualTypeOf<{
    title: string;
    coverImage: fields.UnfetchedRelation<builtins.MediaValue>;
  }>();
});
...
test("collection multiRelation", () => {
  ...
  expectTypeOf(test).toEqualTypeOf<{
    title: string;
    tags: fields.UnfetchedMultiRelation<typeof tags>;
  }>();
});
...
test("collection with user", () => {
  ...
  expectTypeOf(test).toEqualTypeOf<{
    title: string;
    author: fields.UnfetchedRelation<typeof builtins.User>;
  }>();
});
